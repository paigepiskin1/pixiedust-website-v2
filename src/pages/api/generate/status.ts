...
// NOTE: we intentionally do NOT delete Portrait Library assets after a generation.
// The same uploaded reference (one `asset://` id) is reused across parallel jobs
// (quantity > 1) and re-generations in a session, so deleting on the first
// completion made later jobs fail with "asset not found". Assets are left in place;
// retention is handled out-of-band (manual purge / future periodic sweep).
