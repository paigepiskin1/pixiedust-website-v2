...
import { HERO_KEY, type HeroPick } from "../../../lib/hero";
...
async function load(db: import("@cloudflare/workers-types").D1Database): Promise<HeroPick[]> {
  try { const p = JSON.parse((await getSetting(db, HERO_KEY)) || "[]"); return Array.isArray(p) ? p : []; } catch { return []; }
}
...
export async function POST({ request, locals }: APIContext) {
  ...
  switch (b.action) {
    case "add_template": {
      ...
      if (picks.some((p) => p.kind === "template" && p.id === b.id)) return json({ error: "Already in the hero" }, 400);
    ...
    case "add_custom": {
      ...
      picks.push({
        ...
        kicker: b.kicker ? String(b.kicker).slice(0, 60) : undefined,
        href: b.href ? String(b.href).slice(0, 300) : "#",
        url: String(b.url),
        mediaType: b.mediaType === "video" ? "video" : "image",
        cr: Number.isFinite(Number(b.cr)) ? Number(b.cr) : undefined,
      });
      break;
    }
    case "remove": {
  ...
  await setSetting(env.DB, HERO_KEY, JSON.stringify(picks));
  await auditAdmin(env.DB, actor, "hero.set", "hero", b.action, { count: picks.length });
